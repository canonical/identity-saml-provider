...
Requires database connection via SAML_PROVIDER_DB_* environment variables:
  ...
  SAML_PROVIDER_DB_PASSWORD (default: saml_provider)`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		return runJanitorPendingRequests(cmd, cmd.Context(), janitorBatchSize, janitorFormat)
	},
}
...
func runJanitorPendingRequests(cmd *cobra.Command, ctx context.Context, batchSize int, format string) error {
	formatter, err := newJanitorFormatter(format)
	if err != nil {
		return err
	}
	...
	for {
		deleted, err := pendingSvc.CleanupExpired(loopCtx, batchSize)
		if err != nil {
			// Gracefully handle context cancellation/timeouts to report partial progress
			if errors.Is(err, context.DeadlineExceeded) || errors.Is(err, context.Canceled) {
				break
			}
			return fmt.Errorf("failed to execute cleanup batch: %w", err)
		}
	...
	if err := formatter.FormatPendingRequests(cmd.OutOrStdout(), totalDeleted); err != nil {
