...
- [ ] 1.1 Create `migrations/003_add_persistent_nameids.sql`
...
- [ ] 2.2 If a typed domain error for "missing canonical subject"
...
- [ ] 2.3 Unit test the accessor in `internal/domain/session_test.go`:
...
- [ ] 3.1 In `internal/repository/interfaces.go`, add the
...
- [ ] 3.2 Create `internal/repository/postgres/persistent_nameid.go`
  ...
  existing repo style.
- [ ] 3.3 Implement `GetOrCreate` as a single statement:
...
- [ ] 3.4 Wrap any Postgres error with context via `fmt.Errorf`
...
- [ ] 4.1 In `internal/service/attribute_mapping.go`, replace
...
- [ ] 4.2 Implement the four format branches in `resolveNameID`:
  ...
  - default (unspecified / empty / unknown URN): preserve the
    ...
    this change.
- [ ] 4.3 Update `mappingService` struct to add a field
...
- [ ] 4.4 In `ApplyMapping`, after `BuildUserAttributes`, extract
...
- [ ] 4.5 Add the structured logging required by the spec's
...
- [ ] 5.1 In `internal/service/attribute_mapping_test.go`, replace
...
- [ ] 5.2 Add table-driven cases for `resolveNameID`:
...
- [ ] 5.3 Add an `ApplyMapping`-level test that demonstrates the
...
- [ ] 5.4 Verify (via captured logs) that the INFO line on
...
- [ ] 6.1 If the project gains or already has a Postgres
...
- [ ] 6.2 If no Postgres harness exists in the repository today,
...
- [ ] 8.2 Grep the codebase for any remaining references to
  `getNameIDValue` and update or delete them. The function should
  no longer exist after this change.

## 9. Verification suite

- [ ] 9.1 Run `make fmt` and commit any formatting deltas.
- [ ] 9.2 Run `make build`; address any compile errors.
- [ ] 9.3 Run `make lint`; resolve all warnings (zero-tolerance per
  the repository constitution).
- [ ] 9.4 Run `make test`; ensure every spec scenario in
...
- [ ] 10.4 Use a conventional-commit footer to communicate the
...
- [ ] 10.5 In the PR description, document the rollback caveat
